...
import { Clock, Calendar, Zap, Settings2, Layers, ChevronDown, CalendarDays, CalendarOff, Columns, FileText, Shield, ShieldCheck, CheckSquare } from "lucide-react";
...
const EVENT_SENSOR_OPTS = [
  { value: "file_watcher", label: "File Sensor",       desc: "Trigger when a file appears or changes in a path" },
  { value: "s3_event",     label: "S3 / ADLS Sensor",  desc: "Trigger on object create/update in cloud storage" },
  { value: "db_sensor",    label: "SQL Sensor",         desc: "Trigger when a SQL query returns true" },
  { value: "sftp_sensor",  label: "SFTP Sensor",        desc: "Trigger when a file arrives on an SFTP server" },
  { value: "api_webhook",  label: "API / Webhook",      desc: "Trigger via an inbound HTTP call" },
  { value: "upstream_job", label: "External Task",       desc: "Wait for another DAG / task to complete" },
];
      ...
      <SectionCard icon={Layers} title="Advanced Pipeline Features" collapsible defaultOpen={false}>
        ...
        {formData.enable_advanced && (
          <div className="space-y-2 pt-1">
            ...
            {[
              { key: "column_mapping", label: "Column Mapping & Transformations", icon: Columns, desc: "Rename, cast, reorder, and apply expressions to columns", locked: true },
              { key: "data_cleansing", label: "Data Cleansing", icon: Zap, desc: "Trim, normalize, deduplicate, and clean data" },
              { key: "data_quality", label: "Data Quality Rules", icon: FileText, desc: "Null checks, uniqueness, range validation, row-count thresholds" },
              { key: "security", label: "Security & Masking", icon: Shield, desc: "Column-level masking, encryption, classification, and access control" },
              { key: "sla", label: "SLA Configuration", icon: ShieldCheck, desc: "Dataset-level SLA targets, breach thresholds, and alerting" },
            ].map(feat => {
